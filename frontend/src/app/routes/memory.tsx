...
import { DECIDED_STROKE, LEGEND, RETIRED_STROKE, edgeAppearance, isRetired, partAppearance } from './memoryGraph'
...
export default function MemoryRoute() {
  ...
  const nodeCanvasObject = useCallback((node: MemoryNode, context: CanvasRenderingContext2D, globalScale: number) => {
    ...
    if (node.kind === 'part' && node.part) {
      ...
      // The fill carries the lifecycle, and a word backs it up. See `memoryGraph.ts` for
      // why the ring on its own was not readable at the zoom this graph settles at.
      const look = partAppearance(node.part.lifecycle, node.part.history.length + node.part.findings.length)
      context.fillStyle = look.fill; context.beginPath(); context.arc(x, y, radius, 0, Math.PI * 2); context.fill()
      if (look.stroke) {
        context.strokeStyle = look.stroke
        context.lineWidth = look.strokeWidth / globalScale; context.beginPath(); context.arc(x, y, radius + 3 / globalScale, 0, Math.PI * 2); context.stroke()
      }
      if (look.badge) {
        context.fillStyle = DECIDED_STROKE; context.beginPath(); context.arc(x + radius, y - radius, 6 / globalScale, 0, Math.PI * 2); context.fill()
        context.fillStyle = '#001f24'; context.font = `${8 / globalScale}px JetBrains Mono`; context.textAlign = 'center'; context.textBaseline = 'middle'; context.fillText(String(look.badge), x + radius, y - radius)
      }
    } else {
      const side = 12; context.fillStyle = '#424a52'; context.fillRect(x - side / 2, y - side / 2, side, side)
    }
    if (globalScale >= 0.7) {
      const below = y + (node.kind === 'part' && node.part ? partRadius(node.part) + 7 : 13) / globalScale
      context.fillStyle = '#dce4e5'; context.font = `${11 / globalScale}px JetBrains Mono`; context.textAlign = 'center'; context.textBaseline = 'top'
      context.fillText(node.kind === 'line' ? compactName(node.label) : node.label, x, below)
      // The lifecycle in words under the part number. WCAG 1.4.1: the colour must never be
      // the only carrier, and a screenshot of this graph travels without any legend.
      const note = node.kind === 'part' && node.part ? partAppearance(node.part.lifecycle, 0).note : null
      if (note) {
        context.fillStyle = RETIRED_STROKE; context.font = `${9 / globalScale}px JetBrains Mono`
        context.fillText(note, x, below + 12 / globalScale)
      }
    }
    context.restore()
  ...
  const linkCanvasObject = useCallback((link: MemoryLink, context: CanvasRenderingContext2D, globalScale: number) => {
    ...
    // Dashed grey means "was on this board, then replaced" — a solid edge would claim the
    // part is still in that BOM, which is the one thing this edge exists to say it is not.
    // Dashed warm and heavier means "is on this board and going away", which used to be
    // drawn exactly like a healthy edge and is the whole subject of a change notice.
    const retiredEndpoint = [source, target].some((end) => end.kind === 'part' && end.part ? isRetired(end.part.lifecycle) : false)
    const look = edgeAppearance({ historical: Boolean(link.historical), retiredEndpoint })
    context.save(); context.globalAlpha = (visible ? look.alpha : 0.06) * progress; context.strokeStyle = look.colour; context.lineWidth = look.width / globalScale
    if (look.dashed) context.setLineDash([4 / globalScale, 4 / globalScale])
